...
fn make_key_deterministic() {
    let k1 = make_key(0x0A00_0001, 443);
    let k2 = make_key(0x0A00_0001, 443);
    assert_eq!(k1, k2);
}
...
fn make_key_ip_only() {
    ...
    assert_eq!(make_key(1, 0), 1 << 16);
    assert_eq!(make_key(0x1234_5678, 0), (0x1234_5678u64) << 16);
}
...
fn make_key_formula() {
    ...
    assert_eq!(make_key(src_ip, src_port), expected);
}

#[test]
fn make_key_max_bounds() {
    let k = make_key(u32::MAX, u16::MAX);
    assert_eq!(k, (u32::MAX as u64) << 16 | (u16::MAX as u64));
}

#[test]
fn make_key_different_ips_different_keys() {
    let k1 = make_key(0x0A00_0001, 80);
    let k2 = make_key(0x0A00_0002, 80);
    assert_ne!(k1, k2);
}
