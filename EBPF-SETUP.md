...
| `HUGINN_EBPF_DST_PORTS`                 | `443` or `80,443`    | One or two destination ports whose SYNs are captured, comma-separated. Replaces `HUGINN_EBPF_DST_PORT`. One port for HTTP-only, HTTPS-only, or dual listen with effective `listen.https_redirection = true` (capture `port_tls`). Use both ports only for dual listen with `listen.https_redirection = false`, when HTTP is proxied. `0`, an empty list, a duplicate, or a third port makes the agent exit. `0` does not disable the filter. |
