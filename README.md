...
- **Configuration** - One file, **[TOML or YAML](SETTINGS.md)** (picked from the extension). **Dynamic** sections (
  routes,
  backends, pools, headers, security filters, rate limits, …) **hot-reload** on SIGHUP or file-watch; **static** sections
...
- **TLS (JA4_ro)**: `x-tls-ja4-ro` - original ClientHello order, raw hex (FoxIO JA4_ro)
- **TLS (JA4_s1)**: `x-tls-ja4-s1` - cipher suites and extensions sorted, SHA-256 hashed, session extensions
  excluded (huginn-net-tls Stable v1)
- **TLS (JA4_rs1)**: `x-tls-ja4-rs1` - raw hex, session extensions excluded (huginn-net-tls Stable v1)
...
- **Spoofing Signature Detection**: `x-fingerprint-spoofing-detected` - If the client sends any
  ...
  `x-tls-ja4`, `x-tls-ja4-r`, `x-tls-ja4-o`, `x-tls-ja4-ro`, `x-tls-ja4-s1`, `x-tls-ja4-rs1`,
...
x-tls-ja4-ro:   t13d3112h2_1302,1303,..._0000,..._0403,...

# TLS - stable V1 (huginn-net-tls)
x-tls-ja4-s1:  t13d3111h2_e8f1e7e78f70_375ca2c5e164
x-tls-ja4-rs1: t13d3111h2_002f,0033,...,ccaa_000a,..._0403,...
