...
fn test_error_type_mapping() {
    assert_eq!(HttpError::InvalidHostInRequestHeader.error_type(), "invalid_host");
    assert_eq!(HttpError::NoMatchingBackend.error_type(), "no_matching_backend");
    assert_eq!(HttpError::NoUpstreamCandidates.error_type(), "no_upstream_candidates");
    assert_eq!(
...
fn test_status_code_conversion() {
    ...
    assert_eq!(StatusCode::from(HttpError::NoUpstreamCandidates), StatusCode::NOT_FOUND);
    assert_eq!(
