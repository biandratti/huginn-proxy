...
/// block is required when any domain sets `acme = true` (validated in `config/loader.rs`).
///
/// Only exact hosts are supported (no wildcards, no catch-all); wildcards are delegated
/// to an external issuer (cert-manager / Caddy / lego) consumed via `cert_path`/`key_path`.
