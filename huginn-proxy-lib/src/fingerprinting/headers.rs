...
pub mod names {
    ...
    /// Header name for TLS JA4_ro fingerprint injection (FoxIO naming)
    ///
    /// JA4_ro: cipher suites and extensions in original ClientHello, raw (not hashed) hex values.
    /// Combines original order and raw values - maximum detail for analysis.
    /// It is injected for all TLS connections when fingerprinting is enabled.
    pub const TLS_JA4_RO: &str = "x-tls-ja4-ro";

    /// Header name for TLS JA4_s1 stable fingerprint injection (huginn-net-tls Stable v1)
    ///
    /// JA4_s1: cipher suites and extensions sorted, SHA-256 hashed, session extensions
    /// excluded (`S1_SESSION_EXTENSIONS`: padding 0x0015, session ticket 0x0023,
    /// pre-shared key 0x0029, early_data 0x002a).
    ...
    /// Header name for TLS JA4_rs1 stable fingerprint injection (raw variant)
    ///
    /// JA4_rs1: same session-extension filtering as JA4_s1, cipher suites and extensions
    /// sorted, raw (not hashed) hex values.
    /// It is injected for all TLS connections when fingerprinting is enabled.
    pub const TLS_JA4_RS1: &str = "x-tls-ja4-rs1";
