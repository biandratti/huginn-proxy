...
use huginn_ebpf_agent::config::{
    ConfigError, DEFAULT_BURST, DEFAULT_WINDOW_SECONDS, SynRateLimit, env, from_env,
};
...
fn rate_limit_disabled_by_default_and_ignores_garbage_burst() {
    ...
    let cfg = parse_ok(required_with(vec![
        (env::RATE_LIMIT_BURST, "not-a-number"),
...
fn rate_limit_enabled_converts_burst_and_window_seconds() {
    let cfg = parse_ok(required_with(vec![
        ...
        (env::RATE_LIMIT_BURST, "2000"),
        (env::RATE_LIMIT_WINDOW_SECONDS, "2"),
    ]));
    assert!(cfg.rate_limit.enabled());
...
fn rate_limit_enabled_without_burst_falls_back_to_the_default() {
    let cfg = parse_ok(required_with(vec![(env::RATE_LIMIT_ENABLED, "true")]));
    assert!(cfg.rate_limit.enabled());
...
fn rate_limit_unusable_burst_is_rejected() {
    ...
    for burst in ["0", "65535", "131070", "5000000000", "not-a-number", "-1"] {
        let result = from_env(required_with(vec![
            (env::RATE_LIMIT_ENABLED, "true"),
            (env::RATE_LIMIT_BURST, burst),
        ]));
        assert!(
            matches!(
                result,
                Err(ConfigError::Invalid { ref name, .. })
                    if name == env::RATE_LIMIT_BURST
            ),
...
fn rate_limit_unusable_window_seconds_is_rejected() {
    ...
    for window in ["0", "not-a-number", "-3", "3601", "10000000000000"] {
        let result = from_env(required_with(vec![
            (env::RATE_LIMIT_ENABLED, "true"),
            (env::RATE_LIMIT_BURST, "500"),
            (env::RATE_LIMIT_WINDOW_SECONDS, window),
        ]));
        assert!(
            matches!(
                result,
                Err(ConfigError::Invalid { ref name, .. })
                    if name == env::RATE_LIMIT_WINDOW_SECONDS
            ),
...
fn rate_limit_accepts_the_longest_usable_window() {
    ...
    let cfg = parse_ok(required_with(vec![
        (env::RATE_LIMIT_ENABLED, "true"),
        (env::RATE_LIMIT_BURST, "500"),
        (env::RATE_LIMIT_WINDOW_SECONDS, "3600"),
    ]));
...
fn rate_limit_invalid_enabled_value_is_rejected() {
    let result = from_env(required_with(vec![(env::RATE_LIMIT_ENABLED, "maybe")]));
    assert!(
        matches!(
            result,
            Err(ConfigError::Invalid { ref name, .. })
                if name == env::RATE_LIMIT_ENABLED
        ),
...
fn rate_limit_enabled_tolerates_case_and_surrounding_whitespace() {
    ...
    for raw in ["true", "TRUE", "True", " true ", "true\n"] {
        let cfg = parse_ok(required_with(vec![(env::RATE_LIMIT_ENABLED, raw)]));
...
fn a_rejected_value_names_itself_and_its_reason() {
    ...
    let result = from_env(required_with(vec![
        (env::RATE_LIMIT_ENABLED, "true"),
        (env::RATE_LIMIT_BURST, "500"),
        (env::RATE_LIMIT_WINDOW_SECONDS, "4000"),
    ]));
    match result {
        Err(ConfigError::Invalid { name, value, reason }) => {
            assert_eq!(name, env::RATE_LIMIT_WINDOW_SECONDS);
