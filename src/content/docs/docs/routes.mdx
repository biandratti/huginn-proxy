...
- **`cert_path`**, **`key_path`**: required together on every domain, catch-all included, when `listen.port_tls` is set. Domains may share the same pair of files. Omit both only on an HTTP-only listener. Omitting them to inherit the catch-all certificate is rejected.
- **`client_ca_path`**: optional client-CA bundle; when set, this domain requires **mTLS** (see [TLS](/huginn-proxy/docs/tls/#client-authentication-mtls))
