...
**TLS** termination options live under **`[tls]`**; **mTLS** is per-domain via `client_ca_path`. See [TLS](/huginn-proxy/docs/tls/).
...
When the proxy sits behind a load balancer or ingress, client IPs in `X-Forwarded-For` can be trusted for real-IP resolution. **`[security.trusted_proxies]`** is a **table** (`cidrs` + `insecure`): it applies to **all** requests and is **not** overridable per domain or route.

When `cidrs` is set and the peer IP is in that list, the proxy walks `X-Forwarded-For` right-to-left and uses the first IP **not** in the trusted list as the real client IP. This real IP drives rate limiting (`limit_by = "ip"` or `"combined"`) and IP filtering. The same list gates [PROXY protocol](/huginn-proxy/docs/listen/#proxy-protocol) headers: only trusted peers may supply a PROXY address.

`insecure = true` trusts every peer (Traefik-style) and replaces listing `0.0.0.0/0`. Over-broad CIDRs warn on load / `--validate`; with `--strict`, warnings fail CI. When `cidrs` is empty and `insecure` is false (default), the non-forgeable TCP peer IP is used directly. **Dynamic** (hot-reloadable).
