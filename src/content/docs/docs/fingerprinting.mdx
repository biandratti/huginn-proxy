---
title: Fingerprinting
description: TLS JA4, HTTP/2 Akamai, and TCP SYN headers.
sidebar:
  order: 6
---

import { Tabs, TabItem } from '@astrojs/starlight/components';

Huginn Proxy passively extracts fingerprints and forwards them as request headers. Analysis and blocking decisions belong on the backend.

For a full feature overview (protocols, limitations, metrics), see [Features](https://github.com/biandratti/huginn-proxy/blob/master/FEATURES.md) on GitHub.

Global toggles live under **`[fingerprint]`** (**static**). Per-route control uses **`fingerprinting`** on each route (TLS + HTTP/2 only; TCP SYN stays global).

## TLS [JA4]

Derived from the TLS ClientHello (via [huginn-net-tls](https://crates.io/crates/huginn-net-tls)):

| Header | Role |
| --- | --- |
| `x-tls-ja4` | Sorted cipher suites and extensions, SHA-256 hashed (FoxIO JA4) |
| `x-tls-ja4-r` | Sorted, raw hex (JA4_r) |
| `x-tls-ja4-o` | Original ClientHello order, hashed (JA4_o) |
| `x-tls-ja4-ro` | Original order, raw hex (JA4_ro) |
| `x-tls-ja4-s1` | [JA4_s1](https://github.com/biandratti/huginn-net/blob/master/huginn-net-tls/JA4S1.md): session extensions excluded, sorted, hashed |
| `x-tls-ja4-rs1` | JA4_s1 raw |

`JA4_s1` / `JA4_rs1` are huginn-only (`stable-v1`). They are not in the FoxIO JA4 spec: official `JA4` / `JA4_r` / `JA4_o` / `JA4_ro` stay unchanged. Same ClientHello, but padding, session_ticket, pre_shared_key, and early_data are dropped before the hash so fresh and resumed handshakes share one key. Spec: [JA4S1.md](https://github.com/biandratti/huginn-net/blob/master/huginn-net-tls/JA4S1.md).

TLS fingerprints are usually **once per TLS session**. For debugging per-connection variation (e.g. extension order randomization), you may need to force new connections or adjust ALPN / keep-alive. That is not generally recommended for production.

**Enable or disable globally** with `tls_enabled`:

<Tabs syncKey="huginn-config">
  <TabItem label="YAML">

```yaml
fingerprint:
  tls_enabled: true
```

  </TabItem>
  <TabItem label="TOML">

```toml
[fingerprint]
tls_enabled = true   # default: inject JA4 headers when TLS is used
# tls_enabled = false  # no TLS fingerprint headers on any route
```

  </TabItem>
</Tabs>

**Per route:** use `fingerprinting` on the route (see [Per-route](#per-route-toggle)); it turns **TLS and HTTP/2** fingerprint headers on or off together for that prefix.

## HTTP/2 [Akamai]

On HTTP/2 connections only, a compact signature is emitted as `x-http2-akamai` using [huginn-net-http](https://crates.io/crates/huginn-net-http).

**Enable or disable globally** with `http_enabled`:

<Tabs syncKey="huginn-config">
  <TabItem label="YAML">

```yaml
fingerprint:
  http_enabled: true
```

  </TabItem>
  <TabItem label="TOML">

```toml
[fingerprint]
http_enabled = true   # default: Akamai header on h2 when fingerprinting is on
# http_enabled = false  # no HTTP/2 fingerprint header on any route
```

  </TabItem>
</Tabs>

If `tls_enabled` or `http_enabled` is `false`, the corresponding headers are not injected. Per-route `fingerprinting` applies to **both** TLS and HTTP/2 fingerprints for that route.

## TCP SYN [p0f]

When built with `ebpf-tcp`, `tcp_enabled = true`, and the [eBPF agent](/huginn-proxy/docs/ebpf-setup/) is running, a p0f-style string is sent as `x-tcp-p0f` ([huginn-net-tcp](https://crates.io/crates/huginn-net-tcp)).

**Enable or disable globally.** There is **no per-route** TCP SYN toggle:

<Tabs syncKey="huginn-config">
  <TabItem label="YAML">

```yaml
fingerprint:
  tcp_enabled: true
```

  </TabItem>
  <TabItem label="TOML">

```toml
[fingerprint]
tcp_enabled = true
# tcp_enabled = false   # no x-tcp-p0f; proxy still runs without eBPF TCP path
```

  </TabItem>
</Tabs>

**Constraints**

- Linux with eBPF (XDP or TC); not available on macOS or Windows for this path.
- Typically **one signature per TCP connection**, reused across HTTP requests on that connection.

## Per-route toggle

`fingerprinting` on a route enables or disables **TLS (JA4) and HTTP/2 (Akamai)** headers for requests matching that route. **TCP SYN** remains governed only by **`fingerprint.tcp_enabled`** (global).

Example: fingerprints on `/api`, but not on `/static`:

<Tabs syncKey="huginn-config">
  <TabItem label="YAML">

```yaml
domains:
  - host: "example.com"
    routes:
      - prefix: "/api"
        backend: "api:9000"
        fingerprinting: true
      - prefix: "/static"
        backend: "cdn:9000"
        fingerprinting: false

fingerprint:
  tls_enabled: true
  http_enabled: true
  tcp_enabled: true
  max_capture: 65536
```

  </TabItem>
  <TabItem label="TOML">

```toml
[[domains]]
host = "example.com"

[[domains.routes]]
prefix = "/api"
backend = "api:9000"
fingerprinting = true

[[domains.routes]]
prefix = "/static"
backend = "cdn:9000"
fingerprinting = false

[fingerprint]
tls_enabled = true
http_enabled = true
tcp_enabled = true   # still applies to all routes when eBPF is available
max_capture = 65536
```

  </TabItem>
</Tabs>

Example: disable TLS + HTTP/2 fingerprints everywhere via config, but keep the section for documentation:

<Tabs syncKey="huginn-config">
  <TabItem label="YAML">

```yaml
fingerprint:
  tls_enabled: false
  http_enabled: false
  tcp_enabled: true
```

  </TabItem>
  <TabItem label="TOML">

```toml
[fingerprint]
tls_enabled = false
http_enabled = false
tcp_enabled = true   # only TCP SYN headers (if eBPF build + agent)
```

  </TabItem>
</Tabs>

See [Configuration overview](/huginn-proxy/docs/configuration/) for where `[fingerprint]` sits in the file; fields include `max_capture` (HTTP/2 capture cap).
